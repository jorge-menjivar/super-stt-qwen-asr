...
It replaces a Python backend that ran the same models through PyTorch and the
`qwen-asr` package, bundled with a relocatable CPython. What changed for a user:
...
Python backend is the last release's code on `transformers` 4.57.6 and PyTorch
2.14 (CUDA 13.0), in bf16 as it shipped. Medians:
...
SUPER_STT_BACKEND_CACHE_DIR=$(mktemp -d) \
  ./super-stt-backend-qwen-asr export-kernels \
